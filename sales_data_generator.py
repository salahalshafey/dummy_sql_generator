...
def generate_random_sales_data(num_records, sale_id_starting, customer_id_range, employee_id_range,
                               discount_range, date_range, payment_method_id_range, payment_amount_range,
                               sale_details_range, product_unit_id_range, sale_price_range, quantity_range):
    sales_sql = "INSERT INTO `trade`.`sale` (`customer_id`, `employee_id`, `discount`, `date`)\nVALUES\n"
    payments_sql = "INSERT INTO `trade`.`sale_payment` (`sale_id`, `employee_id`, `payment_method_id`, `amount`, `notes`, `payment_date`)\nVALUES\n"
    sale_details_sql = "INSERT INTO `trade`.`sale_detail` (`sale_id`, `product_unit_id`, `sale_price`, `quantity`)\nVALUES\n"
    ...
    for sale_id in range(sale_id_starting, sale_id_starting + num_records):
        ...
        discount = round(random.uniform(discount_range[0], discount_range[1]), 2)
        random_date = start_date + timedelta(seconds=random.randint(0, int((end_date - start_date).total_seconds())))
        sale_date_str = random_date.strftime('%Y-%m-%d %H:%M:%S')

        sales_values.append(f"({customer_id}, {employee_id}, {discount}, '{sale_date_str}')")
        ...
        for _ in range(num_sale_details):
            ...
            sale_details_values.append(f"({sale_id}, {product_unit_id}, {sale_price}, {quantity})")
...
tk.Label(inputs_frame, text="Discount Min", bg="#f0f0f0").grid(row=2, column=0, sticky="w", padx=5, pady=5)
discount_min_entry = tk.Entry(inputs_frame)
discount_min_entry.insert(0, "0.00")
discount_min_entry.grid(row=2, column=1, padx=5, pady=5)

tk.Label(inputs_frame, text="Discount Max", bg="#f0f0f0").grid(row=2, column=2, sticky="w", padx=5, pady=5)
discount_max_entry = tk.Entry(inputs_frame)
discount_max_entry.insert(0, "0.00")
discount_max_entry.grid(row=2, column=3, padx=5, pady=5)
